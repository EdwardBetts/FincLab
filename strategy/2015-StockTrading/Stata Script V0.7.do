...
global Min_Obs = 100 								// Mininum number of observations in each category; if fall below this threshold, small categories will be merged.
...
* Main Program
	...
	disp "*******************************************************"			
		...
		if "$Category_By_Clusters"=="Yes" {
			...
			gen category = 1
			forvalues i = 1/`rows' {
				replace category = `i' + 1 if d_cat`i'==1
			}
			drop d_cat*
...
disp "*******************************************************"	
	...
	* Exclude if F-stats is too low
	...
	file write ret_csv "test_begin,test_end,month,porfolio,sp500, asset_weighted,(monthly returns)" _n		
