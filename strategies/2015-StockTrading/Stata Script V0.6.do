...
global Start_Year 		= 2009 
global Start_Month 		= 6
global Start_Date 		= 1
...
global CRSP_Vars 		= "permno date prc ret sprtrn shrout siccd" 							// Variables to be extracted from the CRSP datasets
...
disp "*******************************************************"	
	...
	* Exclude if F-stats is too low
	...
	foreach len in 1 5 10 25 50 100 {
		use "$Temp_Folder\\ols_results_filtered.dta", clear
		...
		qui{
			...
			preserve 
				...
				qui keep firmid
				qui save "$Temp_Folder\\InSample-Buttom`len'.dta", replace
			restore 

			preserve 
				qui keep if rank_top<=`len' 	// save the top 50 list from in-sample
				qui keep firmid
				qui save "$Temp_Folder\\InSample-Top`len'.dta", replace
			restore 	
			...
			qui {
				...
				// Merge with top 50
				merge m:1 firmid using "$Temp_Folder\\InSample-Top`len'.dta"
				gen TopList = _merge==3
				drop _merge
				
				// Merge with bottom 50
				merge m:1 firmid using "$Temp_Folder\\InSample-Buttom`len'.dta"
				...
				scalar cumul_sp500_ret = round((sp500_end - sp500_start)/sp500_start*100, 0.01)
				...
				sum cumulativeRet
				local avgReturn = round(r(mean)*100, 0.01)	
								
			}
			disp ""
			disp "* Spec: `x' len(`len'), TestSet `yr'-`mon': `avgReturn'% p.a., v.s. S&P 500: " cumul_sp500_ret "%." 
