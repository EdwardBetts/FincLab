...
global BeginYear	 	= 2002									// Set the first year for the rolling process to begin with
global SampleWindow 	= 36 									// in MONTHS; Set number of years for extract samples from
global ForecastWindow 	= 12 									// in MONTHS; Set number of years to forecast the model fitness
...
/**/
	* save Coefficients from R2
		...
		if $NumSpecsToRun==1 {
			mat RSq = RSq , ProductRef , C1_`ithvar' 
			global Varname = "firmID permno DefaultModel"
		}
		else {		
...
disp "*******************************************************"	
	...
	disp "S5.1 Compare the correlation of Alphas"
		use "$Data\\Sample_R2.dta", clear
		drop if r2<0.6
		merge 1:1 firmID using "$Data\\Sample_F_test_f.dta"
		keep if _merge==3
		drop _merge
		drop if ftest>0.1
		merge 1:1 firmID using "$Data\\Sample_Alpha_0.dta"
		keep if _merge==3
		drop _merge
	...
	foreach len in 10 25 50 100 {
		...
		*file write tableRoll "Long/Short Portfolio = Long Top `len' firms & Short Bottom `len' firms" _n
		...
		foreach x of global VarName {
			...
			preserve 
				qui keep if rank_`x'_bottom<=`len' // save the bottom 50 list from in-sample
				qui keep permno
				qui save "$Data\\InSample-Buttom`len'-`x'.dta", replace
			restore 

			preserve 
				qui keep if rank_`x'_top<=`len' 	// save the top 50 list from in-sample
				qui keep permno
				qui save "$Data\\InSample-Top`len'-`x'.dta", replace
			restore 	
