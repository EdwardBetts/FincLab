...
global Temp_Folder 		= "D:\Workspace"									// Location to which temporary files are generated
...
if $NumSpecsToRun==1 {
	...
	preserve
		...
		save "$Temp_Folder\\Sample_`matname'_`ithvar'.dta", replace
	restore
end program	
...
***
	...
	*S1.1 Load data from Lippers Tass
		if "$LoadCRSP"=="Yes" {
			...
			forvalues yr = `bYr'/`endYr' {
				append using "$CRSP_Folder\\`yr'.dta"
			}
		...
		* remove duplicates
		...
		save "$Temp_Folder\\CRSP-$beginYear-$endYear.dta", replace
	...
	*S1.3 Merge with factor variables
		...
		*prepare_factors
		...
		use "$Temp_Folder\\CRSP-$beginYear-$endYear.dta", clear
	...
	*S1.4 Generate some variables
	...
	/*
		...
		save "$Temp_Folder\\Panel.dta", replace 		
		}

	disp "*******************************************************"		
	disp " S2. Impute Estimated Market Cap "
	disp "*******************************************************"			
		use "$Temp_Folder\\Panel.dta", clear
...
disp "*******************************************************"	
	disp "* S4.1. Dropping observations s.t. criteria"
		...
		qui gen fail = 0
		qui replace fail = 1 if inSample==1 & ( exRet==. | fac1==. | fac2==. | fac3==. | fac4==. | fac5==. | fac6==. | fac7==. | fac8==. )
		qui replace fail = 1 if postSample==1 & ( exRet==. | fac1==. | fac2==. | fac3==. | fac4==. | fac5==. | fac6==. | fac7==. | fac8==. )
	...
	*S4.2a Equal-weighted Index (Un-Orthogonalised); Eq. (1)
		preserve
			...
			merge m:1 month using "$Temp_Folder\\Factors.dta"
			drop if _merge==2
			drop _merge
	...
	*S4.2b Value-weighted Index; Eq. (2)
	...
	save "$Temp_Folder\\SampleData.dta", replace
	
disp "*******************************************************"	
disp " S4. OLS Regressions "
disp "*******************************************************"	
	use "$Temp_Folder\\SampleData.dta", clear
	...
	*S5.1 Prepare Data
		merge m:1 date month category inSample using "$Temp_Folder\\inSample_EWO.dta"
		drop _merge
		ChangeVarNames
		merge m:1 date month category inSample using "$Temp_Folder\\inSample_VWO.dta"
		drop _merge
		ChangeVarNames
		merge m:1 date month category postSample using "$Temp_Folder\\postSample_EWO.dta"
		drop _merge
		ChangeVarNames
		merge m:1 date month category postSample using "$Temp_Folder\\postSample_VWO.dta"
		drop _merge
		ChangeVarNames
		drop *_temp
		...
		******************************************************
		...
		save "$Temp_Folder\\ols_results.dta", replace

		
disp "*******************************************************"	
disp " S5. Model Fittness "
disp "*******************************************************"	
		use "$Temp_Folder\\ols_results.dta", clear
		...
		*file write tableRoll "Long/Short Portfolio = Long Top `len' firms & Short Bottom `len' firms" _n
		...
		qui{
			...
			qui {
				...
				gen lnRet500 = ln(1+fac6)
				sort permno date
				by permno: egen cumulativeRetE500 = total(lnRet500)
				gen cumulativeRet500 = exp(cumulativeRetE500) - 1
				drop lnRet500 cumulativeRetE500				
				...
				sum cumulativeRet500
				local sp500 = round(((1+r(mean))^(365/`len') - 1)*100, 0.01)
