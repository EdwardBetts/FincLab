"""
    ...
    Notes:
        Construct YQL request URLs.
        The module is NOT working as tested in Oct/2015.
...
# My lib
try:
    from lib.config import Config, Configuration
except:
...
class YahooOAuth(object):
    ...
    def __init__(self, config=Config,
                 logger=None):
        """
        config: Initiatate from /settings/*.conf
        ...
        for index, acc in enumerate(self.config.accounts.sections()):
            for item in self.config.accounts[acc]:
                self.logger.debug('Account Index: ' + str(index) + '\n' +
                                  'Account: ' + acc + '\n' +
                                  'Item: ' + item)

        # Check timestamp to see if token if valid
